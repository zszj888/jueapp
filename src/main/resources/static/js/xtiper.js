...
//构造内部html
    Xclass.pt.html = function () {
        ...
        if (c.model == 'msg') {
            html += '<p>';
            if (c.icon) {
                html += c.iconFlag === true ? '<i class="xtiper_icon xtiper_icon_' + c.icon + ' xtiper_icon_min"></i>' : '<img class="xtiper_icon xtiper_icon_min" th:src="@{' + c.icon + '" />';
            }
            html += c.tip + '</p>';
        }
        //弹幕
        ...
        //页面层
            ...
            if (c.type == 'ready' || c.type == 'noready') { //内容
                ...
                newcontent = '<div class="xtiper_content' + height_css + ' xtiper_over"' + (c.bgcolor ? ' style="background-color:' + c.bgcolor + '"' : '') + '><div class="zw"></div><iframe parentid="' + that.mainid + '" scrolling="' + scrolling + '" allowtransparency="true" frameborder="0" th:src="@{' + c.content + '" style="width:100%; height:100%;"></iframe></div>';
                ...
                for (var i = 0; i < photo.length; i++) {
                    ...
                    li += '<li class="xtiper_photo_li' + (i == xindex ? ' xon' : '') + (that.ifmob === true ? ' xapp' : '') + '" data-xtitle="' + photo[i].title + '"><p style="background-image:url(\'' + xsrc + '\');">' + (xhref ? '<a th:href="@{' + xhref + '" target="_blank">' : '') + '<img th:src="@{' + xsrc + '">' + (xhref ? '</a>' : '') + (i == xindex && that.ifmob === true ? '<span class="xtiper_icon xtiper_icon_load xtiper_photo_load"></span>' : '') + '</p></li>';
                }
                li += '</ul></div>';
            ...
            for (var i = 0; i < c.btn.length; i++) {
                if (btnfun[i]) {
                    if (typeof (btnfun[i]) == 'function') {
                        ...
                        licon = '<a th:href="@{' + href + '"' + target + '><p>' + c.btn[i] + '</p></a>';
                    }
                } else {
                    licon = '<p>' + c.btn[i] + '</p>';
                }
    ...
    Xclass.pt.iconer = function () {
        ...
        if (c.icon) {
            if (c.iconFlag === true) {
                ...
                html = '<img class="xtiper_icon" th:src="@{' + c.icon + '" />';
            }
        }
        return html;
    };
